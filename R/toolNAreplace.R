...
toolNAreplace <- function(x, weight = NULL, replaceby = 0,
  ...
  if (is.magpie(replaceby)) {
    ...
    if (!all(dim(replaceby) == dim(x))) {
      stop("replaceby must be expandable to the exact same size as x in order to work!")
    }
    replaceby <- replaceby[getCells(x), getYears(x), getNames(x)]
  } else if (length(replaceby) != 1) {
    stop("replaceby has a length different than 1. It must be either a MAgPIE object or a single number!")
  }
  if (!is.null(val.rm)) {
    x[x %in% val.rm] <- NA
  }
  if (!is.null(weight)) {
    naRemove <- x * weight * 0 + 1
  } else {
    naRemove <- x * 0 + 1
  }
  x <- x * naRemove
