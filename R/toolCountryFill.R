...
toolCountryFill <- function(x,fill=NA, no_remove_warning=NULL, overwrite=FALSE, verbosity=1, countrylist=NULL,...) {
  ...
  if(length(missing_countries)>0) {
    ...
    if(length(missing_important_countries)>0) {
      ...
      vcat(verbosity," --- ",paste0(names_countries," (",iso_country1[names_countries],") "), show_prefix = FALSE)
    }
    ...
    if(length(missing_important_countries)>0) {
      ...
      vcat(2," --- ",paste0(names_countries," (",iso_country1[names_countries],") "),show_prefix = FALSE)
    }
