...
toolSubtypeSelect <- function(subtype, files) {
  if (is.null(subtype)) {
    stop("Subtype has to be set! Available subtypes are: ", paste(names(files), collapse = ", "))
  }
  if (!(subtype %in% names(files))) {
    stop("Unknown subtype '", subtype, "'! Available subtypes are: ", paste(names(files), collapse = ", "))
  }
