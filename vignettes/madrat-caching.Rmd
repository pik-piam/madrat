...
With `cachetype = "rev"` `retrieveData` will create a new, revision-specific cachefolder and set `setConfig(forcecache = TRUE)` (default is FALSE). Via this approach calculations will start with now cache files at all but created cache files will be read if a calculation is repeated. The forcecache option will in this case make sure that any available cache file which fits the function call is read in, independent of whether the content of the cache file might be outdated or not.
