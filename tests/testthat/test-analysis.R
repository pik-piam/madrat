...
test_that("getMadratInfo properly detects problems", {
  ...
  expect_silent(a <- suppressMessages(getMadratInfo()))
  toolBla <- function() {return(calcOutput("TauTotal"))}
  globalassign("toolBla")
  expect_message(a <- getMadratInfo(), "[warning].*toolBla")
  rm(toolBla,envir = .GlobalEnv)
  calcBla2 <- function() {calcOutput("UnknownType")}
  globalassign("calcBla2")
  expect_warning(a <- getMadratInfo(),"Following functions could not be found in the scope of packages to be checked.: .* calcUnknownType->calcBla2")
})