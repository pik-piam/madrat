...
test_that("Fill with regional average works as expected", {
  ...
  x[] <- 1:2
  expect_message(y <- toolFillWithRegionAvg(x,valueToReplace = 1), "replaced 1 missing value")
  expect_identical(as.vector(y), c(2,2))
  expect_warning(y <- toolFillWithRegionAvg(x,valueToReplace = 1, callToolCountryFill = TRUE),"more than .* percent missing")
