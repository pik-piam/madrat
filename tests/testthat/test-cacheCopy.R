...
test_that("cacheCopy properly detects cache files", {
  ...
  expect_warning(x <- cacheCopy(log, target = file.path(tempdir(), "cachetarget")), "cache files could not be found")
  ...
  writeLines("test", file.path(tempdir(), "calcTauTotal-F933da7d1.rds"))
  logfile <- tempfile()
  writeLines(c(log, paste0('cachefolder -> \"', tempdir(), "\"")), logfile)
  expect_warning({
    x <- cacheCopy(logfile, target = file.path(tempdir(), "cachetarget"))
  }, "More than one cache folder found")
  expect_true(file.exists(file.path(tempdir(), "cachetarget", "calcTauTotal-F933da7d1.rds")))
})
