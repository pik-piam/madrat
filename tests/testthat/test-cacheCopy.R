...
test_that("cacheCopy properly detects cache files", {
  ...
  writeLines("test", paste0(tempdir(),"/calcTauTotal-F933da7d1.rds"))
  logfile <- tempfile()
  writeLines(c(log, paste0('cachefolder -> \"',tempdir(),"\"")), logfile)
  expect_warning(x <- cacheCopy(logfile, target=paste0(tempdir(),"/cachetarget")), "More than one cache folder found")
  expect_true(file.exists(paste0(tempdir(),"/cachetarget/calcTauTotal-F933da7d1.rds")))
})